import {
  getMovieByGenre,
  getMovieByGenreScience_Fiction,
} from "@/services/movie.service";
import Link from "next/link";
import { TbMathGreater } from "react-icons/tb";
import React from "react";

const CardFiction = async () => {
  const movies = await getMovieByGenreScience_Fiction();
  console.log(movies);

  return (
    <div className="px-10 py-3  no-scrollbar ">
      <span className="flex text-white py-4 text-xl">
        Science Fiction movies <TbMathGreater className="m-1 ml-2" />
      </span>
